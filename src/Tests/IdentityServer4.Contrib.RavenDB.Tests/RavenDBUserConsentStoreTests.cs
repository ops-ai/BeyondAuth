...
namespace IdentityServer4.Contrib.RavenDB.Tests
{
    public class RavenDBUserConsentStoreTests : RavenIdentityServerTestBase
    {
        ...
        public RavenDBUserConsentStoreTests(ITestOutputHelper output)
        {
            ...
            _userConsentStore = new RavenDBUserConsentStore(_loggerFactory.CreateLogger<RavenDBUserConsentStore>(), _documentStore);
        }
        ...
        {
            Assert.Throws<ArgumentException>(() => new RavenDBUserConsentStore(null, _documentStore));
